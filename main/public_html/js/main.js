...
document.getElementById("bookShowtime").addEventListener("click", function () {
    ...
    if (!userName || !showtimeId || !tickets) {
        alert("Please provide your name, select a showtime, and specify the number of tickets.");
        return;
    }

    // Construct the query string with showtime ID, tickets, and user name
    const queryString = queryObjectToString({ showtimeId: showtimeId, tickets: tickets, userName: userName });
    ...
    AJAX.onload = function () {
        if (this.status === 200) {
            ...
            if (responseObj.scheduled) {
                ...
                availableTimes = availableTimes.filter(slot => slot.showtime_id !== parseInt(showtimeId, 10));
...
document.getElementById("modifyShowtime").addEventListener("click", function () {
    const showtimeId = document.getElementById("modifyShowtimeId").value.trim();
    const ticketsAvailable = document.getElementById("modifyTicketsAvailable").value.trim();
    ...
    // Validate inputs
    if (!showtimeId || (!ticketsAvailable && !startTime && !endTime && !theaterId && is3D === undefined)) {
